...
def tokenize(text):
    return [
        tok.text.lower() for tok in spacy_en.tokenizer(text)
...
class Encoder(nn.Module):
    def __init__(self, vocab_size, emb_dim, hid_dim):
        ...
        self.fc = nn.Linear(
            hid_dim * 2, hid_dim
        )  # linear layer combines both directions -> 512
...
class LuongAttention(nn.Module):
    def __init__(self, enc_hid_dim, dec_hid_dim):
        ...
        )  # maps encoder output to same size as decoder's hidden size
        self.dropout = nn.Dropout(0.3)  # Reduce overfitting
    ...
    def forward(self, hidden, encoder_outputs):
        ...
        encoder_outputs = encoder_outputs.permute(
            1, 0, 2
        )  # small optimization for faster batching
...
class Decoder(nn.Module):
    def __init__(self, vocab_size, emb_dim, enc_hid_dim, dec_hid_dim):
        ...
        self.rnn = nn.GRU(
            emb_dim + enc_hid_dim * 2, dec_hid_dim
        )  # GRU update decoder hidden state
        self.fc = nn.Linear(
            dec_hid_dim + enc_hid_dim * 2, vocab_size
        )  # map decoder output to vocab size

    def forward(self, input, hidden, encoder_outputs):
        input = input.unsqueeze(
            0
        )  # Add time dimension for decoder to process sequentially
...
def top_k_sampling(logits, k=10):
    ...
    top_k_probability, top_k_index = torch.topk(
        probability, k
    )  # select top 10 most probably tokens
    top_k_probability = (
        top_k_probability / top_k_probability.sum()
    )  # normalize values to 1
    sampled_index = torch.multinomial(
        top_k_probability, 1
    )  # grab a random sample of 1 of the 10 values
...
with open(os.path.join(data_path, "word2index.pkl"), "rb") as f:
    word2idx = pickle.load(f)
with open(os.path.join(data_path, "index2word.pkl"), "rb") as f:
    idx2word = pickle.load(f)
qa_df = pd.read_csv(os.path.join(data_path, "DATASET_PROCESSED.csv"))
...
# Save the model - create models directory if it doesn't exist
models_dir = os.path.join(project_root, "models")
os.makedirs(models_dir, exist_ok=True)
model_path = os.path.join(models_dir, "chatbot_model_with_attention.pth")
