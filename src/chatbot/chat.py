...
class Chat:
    def __init__(
            self,
            model: Seq2SeqNoAttention | Seq2SeqWithAttention,
    ...
    def get_natt_output(self, input_tensor):
    ...
    def get_att_output(self, input_tensor):
        input_tensor_spacy = input_tensor.unsqueeze(1)
        enc_outputs, enc_hidden = self.model.encoder(input_tensor_spacy)
        dec_input = torch.tensor([self.word2idx[SOS_TOKEN]], device=self.device)
        ...
        with torch.no_grad():
            ...
            for _ in range(20):
                ...
                dec_input = torch.tensor([next_token], device=self.device)
    ...
    def ask(self, user_input: str):
        ...
        input_tensor = torch.tensor(id_sentence, dtype=torch.long, device=self.device)

        if isinstance(self.model, Seq2SeqNoAttention):
            return self.get_natt_output(input_tensor)

        if isinstance(self.model, Seq2SeqWithAttention):
            return self.get_att_output(input_tensor)
